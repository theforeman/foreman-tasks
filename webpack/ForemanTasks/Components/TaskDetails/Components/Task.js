...
class Task extends Component {
  componentDidMount() {
    const { state, timeoutId, refetchTaskDetails } = this.props;
    if (state === 'stopped')
      this.props.taskReloadStart(timeoutId, refetchTaskDetails);
  }

  taskProgressToggle = () => {
    const { timeoutId, refetchTaskDetails, id } = this.props;
    if (this.props.timeoutId) {
      this.props.taskReloadStop(timeoutId);
    } else {
      this.props.taskReloadStart(timeoutId, refetchTaskDetails, id);
    }
  };
          ...
          <Row>
            <Col xs={12}>
              <Button
                ...
                className={`reload-button ${taskReload ? 'spin' : ''}`}
                bsSize="small"
